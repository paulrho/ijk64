//////////////////////////////////////////////////////////////////////////////////
//
// $Id: C64Screen.java,v 1.23 2006/02/14 13:14:47 pgs Exp pgs $
//                                                               
...
// this isn't working
  ...
  //javax.swing.Timer timer1 = new javax.swing.Timer(400, new 
    javax.swing.Timer timer1 = new javax.swing.Timer(250, new ActionListener() {
                                                     public void actionPerformed(ActionEvent event) {
                                                   //scrollscreen();
                                                   // blink cursor
                                                     if (System.currentTimeMillis() - cursflash >= 500) {       //
                                                     cursflash = System.currentTimeMillis(); cursVisible = !cursVisible; screenneedsupdate = true;}
                                                     repaint_ifupdated();}
                                                     }
    );
    timer1.start();
    ...
    // draw all the lines
      ...
      if (bgtrans) {
        if (newoffGraphics != null) {   //here
          newoffGraphics.drawImage(bgImage, 0, 0, 8 * scale * maxX + 50 * scale, 8 * scale * maxY + 50 * scale + topY, 0, 0, 8 * scale * maxX + 50 * scale, 8 * scale * maxY + 50 * scale + topY, this);
          newoffGraphics.drawImage(screenBuffImage, 0, 0, 8 * scale * maxX + 50 * scale, 8 * scale * maxY + 50 * scale + topY, 0, 0, 8 * scale * maxX + 50 * scale, 8 * scale * maxY + 50 * scale + topY, this);
      ...
      // normally
        if (false) {
          g2d.drawImage(screenBuffImage, 0, 0, 8 * scale * maxX + 50 * scale, 8 * scale * maxY + 50 * scale + topY, 0, 0, 8 * scale * maxX + 50 * scale, 8 * scale * maxY + 50 * scale + topY, this);
        } else {
          if (newoffGraphics != null) { //here
...
/// key stuff
  ...
  // first look at it to see if it is just a shift/control/alt event
    ...
    // special codes first
      ...
      if ((char) e.getKeyCode() == 'B' && e.isAltDown() && bgtrans_ability) {
        bgtrans = !bgtrans;
        reshapeScreen();
        return;
      }                         // really needs more than this to trigger
