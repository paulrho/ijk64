// $Id: C64Screen.java,v 1.16 2004/11/30 20:50:55 pgs Exp pgs $
//                                                               
...
class C64Screen extends JFrame implements KeyListener {
	...
	String colourname[] = { "BLACK", "WHITE", "RED", "CYAN", "PURPLE", "GREEN",
			"BLUE", "YELLOW", "ORANGE", "BROWN", "LIGHT RED", "GREY 1",
			"GREY 2", "LIGHT GREEN", "LIGHT BLUE", "GREY 3" };

	int fullcolour[] = { 0xFF000000, // black
			0xFFFFFFFF, // white
	...
	BufferedImage charBuffImage = new BufferedImage(64, 64,
			BufferedImage.TYPE_INT_ARGB);

	BufferedImage screenBuffImage = new BufferedImage(8 * 40 * 2 + 200,
			150 * 8 * 2 + 200, BufferedImage.TYPE_INT_ARGB); //here

	BufferedImage bgBuffImage = new BufferedImage(8 * 40 * 2 + 200,
			150 * 8 * 2 + 200, BufferedImage.TYPE_INT_ARGB); //here
	...
	public C64Screen(String title) {
		...
		if (bgtrans_ability) {
			newoffImage = createImage(8 * 40 * 2 + 200 + 1600, 150 * 8 * 2
					+ 200 + 1200);
	...
	public void reshapeScreen() {
		...
		setSize(scale * maxX * 8 + 50 * scale, scale * maxY * 8 + 50 * scale
				+ topY);
		offGraphics = (Graphics2D) screenBuffImage.getGraphics();
		if (!bgtrans) { // here
			offGraphics.setColor(new Color(borderColour, true));
			offGraphics.fillRect(0, 0, scale * maxX * 8 + 50 * scale, scale
					* maxY * 8 + 50 * scale + topY);
			offGraphics.setColor(new Color(backgroundColour, true));
			offGraphics.fillRect(25 * scale, 25 * scale + topY, scale * maxX
					* 8, scale * maxY * 8);
		} else {
		...
		if (rows >= 25 && rows <= MAXmaxY) {
			...
			if (C64Screen.maxY < rows) {
				...
				for (int j = 0; j < rows; ++j) { //move the last out to the new last output
					for (int i = 0; i < maxX; ++i) {
						screenchar[i][j] = screenchar[i][j
								+ (C64Screen.maxY - rows)];
						screencharColour[i][j] = screencharColour[i][j
								+ (C64Screen.maxY - rows)];
					}
				}
	...
	public void create_screen_updater() {
		...
		javax.swing.Timer timer1 = new javax.swing.Timer(250,
				new ActionListener() {
		...
		if (offGraphics != null) {
			...
			if (modImage != null && modImage2 != null) { // hold off until ready
				for (int j = 0; j < maxY; ++j) {
					for (int i = 0; i < maxX; i++) {
						...
						if (pscreenchar[i][j] == screenchar[i][j]
							...
							if (scale == 1) {
								drawchar(screenchar[i][j], 0 + 25 * scale + i
										* 8 * scale, j * 8 * scale + 25 * scale
										+ topY, screencharColour[i][j]);
							} else {
								drawchar2(screenchar[i][j], 0 + 25 * scale + i
										* 8 * scale, j * 8 * scale + 25 * scale
										+ topY, screencharColour[i][j]);
							}
						}
				...
				if (cursEnabled && cursVisible) {
					if (pscreenchar[cursX][cursY] == (char) (' ' + 128)
						...
						if (scale == 1) {
							drawchar((char) (' ' + 128), 0 + 25 * scale + cursX
									* 8 * scale, cursY * 8 * scale + 25 * scale
									+ topY, cursColour);
						} else {
							drawchar2((char) (' ' + 128), 0 + 25 * scale
									+ cursX * 8 * scale, cursY * 8 * scale + 25
									* scale + topY, cursColour);
						}
					}
				}
			}
			...
			if (bgtrans) {
				if (newoffGraphics != null) { //here
					newoffGraphics.drawImage(bgImage, 0, 0, 8 * scale * maxX
							+ 50 * scale, 8 * scale * maxY + 50 * scale + topY,
							0, 0, 8 * scale * maxX + 50 * scale, 8 * scale
									* maxY + 50 * scale + topY, this);
					newoffGraphics.drawImage(screenBuffImage, 0, 0, 8 * scale
							* maxX + 50 * scale, 8 * scale * maxY + 50 * scale
							+ topY, 0, 0, 8 * scale * maxX + 50 * scale, 8
							* scale * maxY + 50 * scale + topY, this);
				...
				// normally
				if (false) {
					g2d.drawImage(screenBuffImage, 0, 0, 8 * scale * maxX + 50
							* scale, 8 * scale * maxY + 50 * scale + topY, 0,
							0, 8 * scale * maxX + 50 * scale, 8 * scale * maxY
									+ 50 * scale + topY, this);
				} else {
					if (newoffGraphics != null) { //here
						// this method seems twice as efficient, but needs proper testing - yes it is faster
						newoffGraphics.drawImage(screenBuffImage, 0, 0, 8
								* scale * maxX + 50 * scale, 8 * scale * maxY
								+ 50 * scale + topY, 0, 0, 8 * scale * maxX
								+ 50 * scale, 8 * scale * maxY + 50 * scale
								+ topY, this);
						g2d.drawImage(newoffImage, 0, 0, this);
					}
				}
	...
	public void printstats() {
		...
		System.out.println("ttime-paint draw      = "
				+ (ttime_paint - ttime_paintm) + " ms");
		...
		for (int i = 0; i < 8; ++i) {
			for (int j = 0; j < 8; ++j) {
				...
				if (dots[posx * 8 + i][posy * 8 + j]) {
					...
					if (bgtrans
							&& bgshadow
							&& (j > 0 && dots[posx * 8 + i][posy * 8 + j - 1] || (i > 0 && dots[posx
									* 8 + i - 1][posy * 8 + j]))) {
						//screenBuffImage.setRGB(xx,yy,backgroundColour & 0x50FFFFFF); //here // 60 worked well //40good, but sometimes too suttle
						screenBuffImage.setRGB(xx, yy,
								backgroundColour & 0x70FFFFFF); //here // 60 worked well //40good, but sometimes too suttle
					} else if (bgtrans)
						screenBuffImage.setRGB(xx, yy,
								backgroundColour & 0x00FFFFFF); //here
					else
					...
					if (scale == 2) {
						...
						screenBuffImage
								.setRGB(xx + 1, yy + 1, backgroundColour);
					}
				}
			}
		}
	...
	public void keyPressed(KeyEvent e) {
		...
		if (e.getKeyChar() != KeyEvent.CHAR_UNDEFINED) {
			...
			else if (e.isControlDown() && keybuf[keybuftop] >= '0'
					&& keybuf[keybuftop] <= '9') {
				keybuf[keybuftop] += 128;
			} else if (e.isControlDown()) {
				keybuf[keybuftop] += 'a' + 128 - 1;
			} else if (tabdown && keybuf[keybuftop] >= '0'
					&& keybuf[keybuftop] <= '9') {
	...
	public String screenInput() {
		while (true) {
			...
			if (ch == 0) { // meta keys
				...
				println();
				return getline();
			} else if (ch == java.awt.event.KeyEvent.VK_BACK_SPACE) {
				backspace();
			} else if (ch >= 256 + '0' && ch <= 256 + '9') {
				setcursColour((short) (ch % 16));
			} else if (ch >= 128 + '0' && ch <= 128 + '9') {
				setcursColour((short) ((ch + 8) % 16));
			} else {
				writechar(ch);
			}
		}
	...
	public void repaint() {
		...
		} else if ((false || !slowdown)
				&& System.currentTimeMillis() - qval > 50) { // was 200 // was 200 again
			// call the real one
			super.repaint();
			qval = System.currentTimeMillis();
		} else {
